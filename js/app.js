...
// Записываем текущее время в баннере
document.addEventListener('DOMContentLoaded', () => {
	if (document.querySelector('.banner-sale')) {
		const days_num = document.querySelector('.banner-sale__day');
		const hours_num = document.querySelector('.banner-sale__hour');
		const mins_num = document.querySelector('.banner-sale__minute');

		const timeCount = () => {
			const time_now = new Date();
			let days = time_now.getDate();
			let hours = `${time_now.getHours()}:`;
			let mins = time_now.getMinutes();
			let minute = '';
			if (mins < 10) {
				minute = `0${mins}`;
			} else {
			...
			days_num.textContent = days;
			hours_num.textContent = hours;
			mins_num.textContent = minute;
		};
...
if ('.main-party__slider') {
	let swiper = new Swiper('.main-party__slider', {
		pagination: {
			el: ".main-party__pagination",
			clickable: true,
		},
		observer: true,
		observeParents: true,
		...
		mousewheel: {
			sensitivity: 0.4,
			eventsTarget: '.main-party__slider'
		},
		// loop: true,
		...
		// Arrows
		navigation: {
			nextEl: '.item-main-party__arrow-next',
		},
